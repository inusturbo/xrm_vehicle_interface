...
XrmZmpNode::XrmZmpNode() : Node("xrm_zmp_interface")
{
  ...
  publish_to_autoware_timer_ = rclcpp::create_timer(
    this, this->get_clock(), period_ns, std::bind(&XrmZmpNode::publishCommands, this));
}
  ...
  const autoware_auto_control_msgs::msg::AckermannControlCommand::SharedPtr msg)
{
  ...
  float steering_tire_rotation_rate =
    msg->lateral.steering_tire_rotation_rate * 180.0f / M_PI;  // rad/s to deg/s
...
void XrmZmpNode::publishCommands()
{
  ...
  velocity_report_msg.heading_rate =
    vehicle_util_->GetDrvSpeedMps() * std::tan(current_steer_wheel) / WHEEL_BASE;  // [rad/s]
