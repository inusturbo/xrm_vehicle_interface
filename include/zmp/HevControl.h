...
public:
  unsigned char * GetDataPtr() { return _byteData; }
  bool AddData(const CANMsg * msg)
  {
    ...
    if (mno == 0) {
      ...
      if (_valid && mno == _mno + 1 && (unsigned int)(msg->DATA[1] << 8 | msg->DATA[2]) == _time) {
        for (int i = 0; i < 4; i++) {
...
public:
   ...
   * @param	firm_version
   */
  virtual void ReceiveVersion(
    char c0, char c1, char c2, char c3, char c4, char c5, char c6, char c7) = 0;
};
...
public:
  ...
  int GetBrakeStatus(
    unsigned char & lamp, unsigned char & blinkLeft, unsigned char & blinkRight,
    unsigned char & mode);
   ...
   * @param[out]	actual シフトポジション現在値(0x00=B, 0x10=D, 0x20=N, 0x40=R, 0x80=P
   * 0x0F=Start 0xFF=Unknown)
   * @param[out]	target シフトポジション指令値(0x00=B, 0x10=D, 0x20=N, 0x40=R, 0x0F=Start)
   * @param[out]	input シフトポジション入力値(0x00=B, 0x10=D, 0x20=N, 0x40=R, 0x80=P,
   * 0x0F=Start, 0xFF=Unknow)
   * @retval	0 成功
   * @retval	0以外 不成功
   */
   ...
   */
  int GetDtcStatus(
    char & data1, char & data2, char & data3, char & data4, char & data5, char & data6,
    char & data7, char & data8);
   ...
   */
  int GetBattInfo(
    float & soc, int & max_temp, int & min_temp, float & max_chg_current,
  ...
  /**
   * @brief		ドアの開閉の状態を取得する
   * @param[out]	doors ドアの状態(0x00=CLOSE, 0x04=トランクOPEN, 0x40=助手席OPEN,
   * 0x80=運転席OPEN)
   * @retval		0 成功
   * @retval		0以外 不成功
   */
   ...
   * @param[out]	データ範囲(0=± 0.7[Ga], 1=± 1.0[Ga], 2=± 1.5[Ga], 3=± 2.0[Ga], 4=± 3.2[Ga],
   * 5=± 3.8[Ga], 6=± 4.5[Ga])
   * @retval		0 成功
   * @retval		0以外 不成功
   */
   ...
   * @param[in]	地磁気範囲(0=± 0.7[Ga], 1=± 1.0[Ga], 2=± 1.5[Ga], 3=± 2.0[Ga], 4=± 3.2[Ga],
   * 5=± 3.8[Ga], 6=± 4.5[Ga])
   * @retval		0 成功
   * @retval		0以外 不成功
   */
...
#define N_OFZ_VALUE 12
   ...
   * @param[in]	target 消去する対象のECU(0=エンジン, 1=ハイブリッド,
   * 2=クルーズコントロール(ブレーキ), 3=バッテリ)
   * @retval		0 成功
   * @retval		0以外 不成功
   */
